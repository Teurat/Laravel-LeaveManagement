...
<div class="mb-5">
    <nav class="flex items-center gap-1.5 text-sm text-slate-500">
        ...
        <span class="font-medium text-slate-900">Edit — {{ $company->name }}</span>
    </nav>
</div>
    ...
    <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
        ...
        <form action="{{ route('companies.update', $company) }}" method="POST" class="space-y-5">
            ...
            <div class="grid grid-cols-2 gap-4">
                <div>
                    <label for="NrEmployees" class="block text-sm font-medium text-slate-700">Target headcount <span class="text-red-500">*</span></label>
                    <input type="number" name="NrEmployees" id="NrEmployees" value="{{ old('NrEmployees', $company->NrEmployees) }}" required min="1"
                           class="mt-1.5 block w-full rounded-lg border-0 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900
                                  shadow-sm ring-1 ring-inset ring-slate-300 focus:ring-2 focus:ring-inset focus:ring-indigo-600 transition-all">
                ...
                <div>
                    ...
                    <input type="number" name="FoundationYear" id="FoundationYear" value="{{ old('FoundationYear', $company->FoundationYear) }}" required
