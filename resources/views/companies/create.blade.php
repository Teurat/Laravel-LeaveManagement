...
<div class="mb-5">
    <nav class="flex items-center gap-1.5 text-sm text-slate-500">
        ...
        <span class="font-medium text-slate-900">Add Company</span>
    </nav>
</div>
...
<div class="grid grid-cols-1 gap-6 lg:grid-cols-3">
    <div class="lg:col-span-2">
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            <h3 class="mb-5 text-sm font-semibold text-slate-900">Company Details</h3>
            <form action="{{ route('companies.store') }}" method="POST" class="space-y-5">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div>
                        <label for="NrEmployees" class="block text-sm font-medium text-slate-700">Employees to register <span class="text-red-500">*</span></label>
                        <input type="number" name="NrEmployees" id="NrEmployees" value="{{ old('NrEmployees') }}" required min="0"
                               placeholder="e.g. 50"
                               class="mt-1.5 block w-full rounded-lg border-0 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900
                                      shadow-sm ring-1 ring-inset ring-slate-300 placeholder:text-slate-400
                                      focus:ring-2 focus:ring-inset focus:ring-indigo-600 transition-all">
                        <p class="mt-1 text-xs text-slate-400">After saving, you'll be guided to register each employee.</p>
                        @error('NrEmployees') <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label for="FoundationYear" class="block text-sm font-medium text-slate-700">Foundation year <span class="text-red-500">*</span></label>
                        <input type="number" name="FoundationYear" id="FoundationYear" value="{{ old('FoundationYear') }}" required
                               min="1800" max="{{ now()->year }}" placeholder="{{ now()->year }}"
                               class="mt-1.5 block w-full rounded-lg border-0 bg-slate-50 px-3.5 py-2.5 text-sm text-slate-900
                                      shadow-sm ring-1 ring-inset ring-slate-300 placeholder:text-slate-400
                                      focus:ring-2 focus:ring-inset focus:ring-indigo-600 transition-all">
                        @error('FoundationYear') <p class="mt-1.5 text-xs text-red-600">{{ $message }}</p> @enderror
                    </div>
                </div>
    ...
    <div>
        <div class="rounded-xl border border-slate-200 bg-white p-6 shadow-sm">
            ...
            <p class="mb-4 text-xs text-slate-500">Upload a CSV file with columns: <code class="rounded bg-slate-100 px-1 py-0.5 font-mono">name, employees, founded_year</code></p>
